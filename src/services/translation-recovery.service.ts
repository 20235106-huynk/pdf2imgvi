...
export function mapGeminiBatchState(state: string | undefined): LocalBatchStatus {
  switch (state) {
...
export interface RetryOptions {
  ...
  deps?: {
    ...
    client?: GeminiBatchClient | Pick<GeminiBatchClient, "uploadFile" | "submitBatch" | "getBatch" | "downloadResults">
    sleep?: typeof abortableDelay
  }
}
...
export async function retryFailedPages(options: RetryOptions): Promise<void> {
  ...
  if (options.fileName !== job.fileName || options.pdf.numPages !== job.totalPages) {
    throw new Error(
